...
var TargetImage = preload("res://images/target.png")
...
func _ready():
	$scoreLabel.modulate = Color(255,255,255)
	...
	Cols2 = range(TargetWidth, ScreenW, TargetWidth) # generate cols array
	for _n in 3: # remove 3 middle columns
		Cols.remove(int(Cols.size()/2))
		Cols2.remove(int(Cols2.size()))
