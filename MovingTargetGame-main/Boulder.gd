...
func end():
	$sprite.texture = load("res://images/boom.png")
	$efecto.start()
	emit_signal("boulder_signal", id) # notify arrow hit target
	
func _on_efecto_tween_completed(_object, _key): # wait end-tween-effect
	queue_free()

func _on_boulder_body_entered(_body):
	end()
